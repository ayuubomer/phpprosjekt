...
// Håndterer innlogging når skjemaet sendes inn
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!filter_var($email, FILTER_VALIDATE_EMAIL)) 
    ...
    // Henter brukerdata fra databasen basert på e-post
    $stmt = db()->prepare('SELECT id, email, password_hash FROM users WHERE email = ?');
    $stmt->execute([$email]);
    $user = $stmt->fetch();
    
    // Verifiserer at bruker eksisterer og passord er riktig
    if (!$user || !password_verify($password, $user['password_hash'])) 
      $err = 'Invalid credentials.';
    else { 
      // Logger inn brukeren og videresender til hovedsiden
      login_user_id((int)$user['id'], $user['email']); 
...
<head>
  ...
  <style>
    ...
    /* Logo-container med gradient bakgrunn */
    .logo{
      width:70px;
      height:70px;
      border-radius:10px;
    ...
    /* Metadata-seksjon (vises når bruker er logget inn) */
    .meta{
      display:flex;
    ...
    /* Hjelpetekst nederst (Sign up-lenke) */
    .helper{ 
      text-align:center;
      margin-top:12px;
      color:var(--muted);
...
<body>
  <div class="wrap">
    ...
    <!-- Vises kun hvis brukeren allerede er logget inn -->
    <?php if ($already): ?>
